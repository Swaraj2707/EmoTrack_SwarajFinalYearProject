...
// import {
//     addExercise
// } from "../../../redux/daily-activities/DailyActivitiesActions";

// import '../../../../css/styles/pages/daily-activities/ExerciseTracker.scss';
...
//         <
//         Card id = "leftCard"
//         className = "whiteBox exerciseCard" >
//         <
...
//             < IconButton onClick = {
//                 start
//             } > < PlayArrowOutlinedIcon className = "play-icon" / > < /IconButton> :
//                 < IconButton onClick = {
//                 stop
//             } > < PauseOutlinedIcon className = "play-icon" / > < /IconButton>
//         } <
//         /CountdownCircleTimer>
...
const ExerciseTracker = () => {
    ...
    useEffect(() => {
        if (weeklyData.length > 0) {
            const td = weeklyData.map(element => Math.ceil(element.activity.exercise));
            setTargetData(td);
        }
    }, [weeklyData]);
    ...
    const formatTime = (val, ...rest) => {
        let value = val.toString().padStart(2, '0');
        if (rest[0] === 'ms') {
    ...
    const start = () => {
        if (!running) {
            setRunning(true);
            setWatch(setInterval(() => setCurrentTimeSec(prevTime => prevTime + 1), 1000));
        }
    };

    useEffect(() => {
        if (currentTimeSec >= 60) {
            setCurrentTimeMin(prevTime => prevTime + 1);
            setCurrentTimeSec(0);
        }
    }, [currentTimeSec]);
    ...
    const saveToLocalStorage = () => {
        const time = `${formatTime(currentTimeMin)}:${formatTime(currentTimeSec)} :: ${new Date().toLocaleTimeString(navigator.language, { hour: '2-digit', minute: '2-digit' })}`;
        localStorage.exerciseTime = localStorage.exerciseTime ? `${time} | ${localStorage.exerciseTime}` : time;
    };
        ...
        <Grid container direction="row" justifyContent="flex-start" alignItems="flex-start" spacing={3}>
            <Grid item xs={12} sm={6}>
                <Card id="leftCard" className="whiteBox exerciseCard">
                    ...
                    >
                        {({ remainingTime }) => (
                            !running ? <IconButton onClick={start}><PlayArrowOutlinedIcon className="play-icon" /></IconButton> :
                                <IconButton onClick={stop}><PauseOutlinedIcon className="play-icon" /></IconButton>
                        )}
                    </CountdownCircleTimer>
                    <div className="stopwatch">{`${formatTime(currentTimeMin)}:${formatTime(currentTimeSec)}`}</div>
                    <div onClick={reset} className="endSessionButton">End Session</div>
                </Card>
            </Grid>
            <Grid item xs={12} sm={6}>
