...
// import '../../../../css/styles/pages/daily-activities/WaterTracker.scss';
...
export default function WaterTracker() {
    ...
    useEffect(() => {
        if (weeklyData.length > 0) {
            const td = weeklyData.map((element) => Math.ceil(element.activity.water));
            setTargetData(td);
            setWaterLevel(td[td.length - 1] || 0);
        }
    }, [weeklyData]);
    ...
    const setDrinkTimeState = () => {
        const drinkTime = localStorage.getItem('drinkTime');
        if (drinkTime) {
            setHistory(drinkTime.split('|'));
        } else {
            setHistory([]);
        }
    };
    ...
    const saveToLocalStorage = () => {
        ...
        const drinkTime = localStorage.getItem('drinkTime');
        if (drinkTime) {
            localStorage.setItem('drinkTime', `${currentTime} | ${drinkTime}`);
        } else {
            localStorage.setItem('drinkTime', currentTime);
        }
    };
