...
const StatCard = ({ obj, dailyData, activeStat, setActiveStat }) => {
    ...
    return (
        <Card
            variant="outlined"
            className="whiteBox daily-stat-card"
...
const DailyStats = ({ activeStat, setActiveStat }) => {
    ...
    return (
        <Grid container direction="row" justifyContent="flex-start" alignItems="center" className="daily-stats">
            {statsList.map((obj) => (
                <Grid item key={`daily-stat-${obj.key}`} xs={12} sm={6} md={4} lg={3}>
                    <StatCard obj={obj} dailyData={dailyData} activeStat={activeStat} setActiveStat={setActiveStat} />
                </Grid>
            ))}
        </Grid>
    );
